    ...
    <section>
        ...
        <b-table
                ...
                <b-table-column :key="column.id" v-bind="column" :sortable="column.sortable">
                    ...
                    <template v-slot="props">
                        ...
                        <span v-else-if="column.field === 'name'" class="has-text-primary is-italic text-xs">
                            <a @click="showReportTreatmentStep(props.row)">
                                {{ props.row[column.field] }}
                            </a>
                        </span>
                        ...
                        <span v-else-if="column.field === 'actions'" class="text-xs">
                            <div class="block">
                                <a @click="editReportTreatmentStep(props.row)" class="tw-inline-block tw-mr-3 text-warning">
                                    <b-icon
                                        pack="fas"
                                ...
                                <a @click="deleteReportTreatmentStep(props.row)" class="tw-inline-block tw-mr-3 text-danger">
                                    <b-icon
                                        pack="fas"
                                        icon="trash"
                                        size="is-small">
            ...
            <template #detail="props">
                ...
                <ReportTreatmentStepItem :reporttreatmentstep_prop="props.row"></ReportTreatmentStepItem>
...
<script>
import ReportTreatmentStepBus from "../reporttreatmentsteps/reporttreatmentstepBus";
...
export default {
    ...
    components: {
        ReportTreatmentStepItem: () => import('../reporttreatmentsteps/item'),
    },
    mounted() {
        ReportTreatmentStepBus.$on('reporttreatmentstep_created', (reporttreatmentstep) => {
            this.addReportTreatmentStepToList(reporttreatmentstep)
        })

        ReportTreatmentStepBus.$on('reporttreatmentstep_updated', (reporttreatmentstep) => {
            this.updateReportTreatmentStepFromList(reporttreatmentstep)
        })
    },
    data() {
        return {
    ...
    methods: {
        showReportTreatmentStep(reporttreatmentstep) {
            window.location = reporttreatmentstep.show_url
        },
        createReportTreatmentStep() {
            ReportTreatmentStepBus.$emit('create_new_reporttreatmentstep')
        },
        editReportTreatmentStep(reporttreatmentstep) {
            ReportTreatmentStepBus.$emit('edit_reporttreatmentstep', { reporttreatmentstep })
        },
        deleteReportTreatmentStep(reporttreatmentstep) {
            this.$swal({
                title: 'Êtes vous sûr ?',
                ...
                if(result.value) {
                    ...
                    axios.delete(`/reporttreatmentsteps/${reporttreatmentstep.uuid}`)
                        // eslint-disable-next-line no-unused-vars
                        .then(resp => {
                            this.removeReportTreatmentStepFromList(reporttreatmentstep)
                        }).catch(error => {
        ...
        addReportTreatmentStepToList(reporttreatmentstep) {
            let reporttreatmentstepIndex = this.reporttreatmentsteps.findIndex(c => {
        ...
        updateReportTreatmentStepFromList(reporttreatmentstep) {
            let stepIndex = this.reporttreatmentsteps.findIndex(s => {
        ...
        removeReportTreatmentStepFromList(reporttreatmentstep) {
            let reporttreatmentstepIndex = this.reporttreatmentsteps.findIndex(s => {
                return reporttreatmentstep.id === s.id
            })
