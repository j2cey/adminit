<template>
    <section>
        <p>
            <b-button size="is-small" type="is-info is-light" @click="createReportServer()">Ajouter</b-button>
        </p>
        ...
        <b-table
                ...
                <b-table-column :key="column.id" v-bind="column" :sortable="column.sortable">
                    ...
                    <template v-slot="props">
                        ...
                        <span v-else-if="column.field === 'name'" class="has-text-primary is-italic text-xs">
                            <a @click="editReportServer(props.row)">
                                {{ props.row[column.field] }}
                            </a>
                        </span>
                        <span v-else-if="column.field === 'osserver'" class="has-text-info is-italic text-xs">
                        ...
                        <span v-else-if="column.field === 'actions'" class="text-xs">
                            <div class="block">
                                <a @click="editReportServer(props.row)" class="tw-inline-block tw-mr-3 text-warning">
                                    <b-icon
                                        pack="fas"
                                ...
                                <a @click="deleteReportServer(props.row)" class="tw-inline-block tw-mr-3 text-danger">
                                    <b-icon
                                        pack="fas"
                                        icon="trash"
                                        size="is-small">
            ...
            <template #detail="props">
                ...
                <ReportServerItem :reportserver_prop="props.row"></ReportServerItem>
        ...
        <AddUpdateReportServer></AddUpdateReportServer>
    </section>
...
import AddUpdateReportServer from "../reportservers/addupdate";
...
export default {
    ...
    components: {
        AddUpdateReportServer,
        ReportServerItem: () => import('../reportservers/item'),
    },
    mounted() {
        ReportServerBus.$on('reportserver_created', (reportserver) => {
            this.addReportServerToList(reportserver)
        })

        ReportServerBus.$on('reportserver_updated', (reportserver) => {
            this.updateReportServerFromList(reportserver)
        })
    },
    data() {
        return {
    ...
    methods: {
        createReportServer() {
            ReportServerBus.$emit('create_new_reportserver')
        },
        editReportServer(reportserver) {
            ReportServerBus.$emit('edit_reportserver', { reportserver })
        },
        deleteReportServer(reportserver) {
            this.$swal({
                title: 'Êtes vous sûr ?',
                ...
                if(result.value) {
                    ...
                    axios.delete(`/reportservers/${reportserver.uuid}`)
                        .then(resp => {
                            this.removeReportServerFromList(reportserver)
                        }).catch(error => {
        ...
        addReportServerToList(reportserver) {
            let reportserverIndex = this.reportservers.findIndex(c => {
        ...
        updateReportServerFromList(reportserver) {
            let stepIndex = this.reportservers.findIndex(s => {
        ...
        removeReportServerFromList(reportserver) {
            let reportserverIndex = this.reportservers.findIndex(s => {
                return reportserver.id === s.id
            })
