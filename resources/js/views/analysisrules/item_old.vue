            ...
            <form role="form">
                ...
                <div class="form-group row">
                    <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success col-sm-6">
                        <input disabled type="checkbox" class="custom-control-input" :id="'notify_when_allowed'+ analysisrule.id" name="notify_when_allowed" placeholder="Alert when allowed" v-model="analysisrule.notify_when_allowed">
                        <label class="custom-control-label" :for="'notify_when_allowed' + analysisrule.id"><span class="text text-xs">Alert when Allowed <i class="far fa-bell"></i></span></label>
                    </div>
                    <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success col-sm-6">
                        <input disabled type="checkbox" class="custom-control-input" :id="'notify_when_broken' + analysisrule.id" name="notify_when_broken" placeholder="Alert when allowed" v-model="analysisrule.notify_when_broken">
                        <label class="custom-control-label" :for="'notify_when_broken' + analysisrule.id"><span class="text text-xs">Alert when Broken <i class="far fa-bell"></i></span></label>
                    </div>
                </div>
                <div class="form-group row">
