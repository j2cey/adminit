...
class TreatmentJob implements ShouldQueue
{
    ...
    /**
     * Indicate if the job should (NOT) be marked as failed on timeout.
     *
     * @var bool
     */
    ...
    public function __construct(Treatment $treatment)
    {
        ...
        //
        $this->onQueue($treatment->service->queue_code->value);
        $treatment->queuing();
        $this->_treatment_id = $treatment->id;

        SystemLog::infoTreatments("TreatmentJob __construct - treatment " . $treatment->type->value . ": " . $treatment->name . "(" . $treatment->id . ")", self::$TREATMENTJOB_LOG_INFO_PART);
    ...
    public function handle()
    {
        ...
        SystemLog::infoTreatments("TreatmentJob handle - treatment " . $treatment->type->value . ": " . $treatment->name . "(" . $treatment->id . ")", self::$TREATMENTJOB_LOG_INFO_PART);
    ...
    public function failed(\Exception|\Error $e = null)
    {
