...
class LocalProtocole  implements IProtocole
{
    ...
    {
        //$treatment_operation = $treatment->operationAddOrGet(TreatmentCodeEnum::PROTOCOL_DISK_GET, $criticalitylevel, $exec_id, $is_last_subtreatment, $can_end_uppertreatment, false, false, array_merge($treatment_payloads, ["protocol"=>"LOCAL"]), null)
        //    ->starting();
        $innertreatment = new InnerTreatment($treatment, TreatmentCodeEnum::PROTOCOL_DISK_GET, $criticalitylevel, $is_last_subtreatment, $can_end_uppertreatment, true, "LOCAL");
        try {
            $fsMgr = new FilesystemManager(app());

            $disk =  $fsMgr->createLocalDriver([
